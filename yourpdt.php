...
<body>
    ...
    <header id="header">
        ...
        <div class="search-wrapper">
            <form action="./yourpdt.php" method="post">
        ...
        <div class="product-card-wrapper">
            ...
            <?php
                ...
                if(isset($_POST["search"])){
                    ...
                    $query = mysqli_query($conn,"SELECT * FROM products WHERE name = '$search' AND accounts_id='$id'");
                    ...
                    if($filter == "None"){
                        $query = mysqli_query($conn,"SELECT * FROM products WHERE accounts_id='$id'");
                        $row = mysqli_fetch_assoc($query);
                    
                    }
                    else if($filter == "Lab Coat"){
                        $query = mysqli_query($conn,"SELECT * FROM products WHERE category='Lab Coat' AND accounts_id='$id'");
                        $row = mysqli_fetch_assoc($query);
                    }

                    else if($filter == "Boiler Suit"){
                        $query = mysqli_query($conn,"SELECT * FROM products WHERE category='Boiler Suit' AND accounts_id='$id'");
                        $row = mysqli_fetch_assoc($query);
                    }
                    else if($filter == "Books"){
                        $query = mysqli_query($conn,"SELECT * FROM products WHERE category='Books' AND accounts_id='$id'");
                        $row = mysqli_fetch_assoc($query);
                    }

                    else if($filter == "Stationary"){
                        $query = mysqli_query($conn,"SELECT * FROM products WHERE category='Stationary' AND accounts_id='$id'");
                        $row = mysqli_fetch_assoc($query);
                    }

                    else if($filter == "Ascend"){
                        $query = mysqli_query($conn,"SELECT * FROM products WHERE accounts_id='$id'ORDER BY name ASC ;");
                        $row = mysqli_fetch_assoc($query);
                    }
                    else if($filter == "Descend"){
                        $query = mysqli_query($conn,"SELECT * FROM products WHERE accounts_id='$id' ORDER BY name DESC;");
                        $row = mysqli_fetch_assoc($query);
                    }
                    else if($filter == "1"){
                        $query = mysqli_query($conn,"SELECT * FROM products WHERE used = 1 AND accounts_id='$id'");
                        $row = mysqli_fetch_assoc($query);
                    }
                    else if($filter == "2"){
                        $query = mysqli_query($conn,"SELECT * FROM products WHERE used = 2 AND accounts_id='$id'");
                        $row = mysqli_fetch_assoc($query);
                    }
                    else if($filter == "3"){
                        $query = mysqli_query($conn,"SELECT * FROM products WHERE used = 3 AND accounts_id='$id'");
                        $row = mysqli_fetch_assoc($query);
                    }
                    else if($filter == "4"){
                        $query = mysqli_query($conn,"SELECT * FROM products WHERE used = 4 AND accounts_id='$id'");
                        $row = mysqli_fetch_assoc($query);
                    }
